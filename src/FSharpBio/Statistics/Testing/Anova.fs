...
module Anova =
    ...
    let oneWayAnova (samples : seq<#seq<float>>) =
        ...
        let means = samples |> Seq.map (fun x -> StatisticalMeasure.mean(x))
        // Step 2. Calculate the overall mean
        let totalMean = StatisticalMeasure.mean(means)
        ...
        //let Ftest = MathNet.Numerics.Distributions.FisherSnedecor(
        let FTest = new FTEST(MSb / MSw, Db, Dw)
